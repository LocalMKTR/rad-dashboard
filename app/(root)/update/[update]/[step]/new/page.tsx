...
export default async function NewStepPage({ params, searchParams }: NewStepPageProps) {
  ...
  const nextPosition = steps && steps.length > 0 ? steps[0].order_position + 1 : 1
  ...
  async function createStep(formData: FormData) {
    ...
    if (!title) {
      redirect(`/build/${buildId}/update/${updateId}/steps/new?error=Title is required`)
    }

    const supabase = await createClient()

    const { error } = await supabase.from("build_update_steps").insert({
      update_id: updateId,
      title,
      description,
    ...
    if (error) {
      redirect(`/build/${buildId}/update/${updateId}/steps/new?error=Failed to create step`)
    }
              ...
              <div className="container mx-auto p-1">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-muted-foreground mt-2">
                    Adding a step to <span className="font-medium">{update.title}</span>
                  </p>
                </div>
                  ...
                  <CardContent>
                    <form action={createStep} className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <Input id="title" name="title" placeholder="e.g., Prepare materials" required />
                      </div>
